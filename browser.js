define({
  exec: function(scope, theurl) { 'use strict';

    var HOMEPAGE = '/c/programs/apps/catExplorer/ixquick/index.html';
    theurl = theurl || HOMEPAGE;

    if (scope._states.opened['catex']) {
      scope._states.opened['catex'].go(theurl);
      return;
    } else {
      scope._states.opened['catex'] = {};
    }
    ...
    var filters = {
      ...
      ,'yahoo.' : ['http://www.potatoland.org/shredder/shredder.html']
      // Webmail
      ,'gmail.' : ['https://www.no-log.org']
      ,'hotmail.' : ['https://help.riseup.net']
      // Réseau Social
      ,'facebook.' : ['https://ello.co', 'https://joindiaspora.com']
      ,'twitter.' : ['http://www.trsst.com', 'http://www.sadtweets.com']
      //,'myspace.' : [go('/c/files/archiveorg/myspace/index.html', 'http://myspace.com')]
      ,'myspace.' : ['http://www.myownspace.fr']
      // Portail vidéo
      ,'youtube.' : ['http://petittube.com', 'http://rutube.ru']
      ,'wat.tv' : ['http://rutube.ru']
      ,'dailymotion.' : ['http://www.ubuweb.com', 'http://full-stream.net']
      // Fournisseurs accès
      ,'orange.fr' : ['http://www.fdn.fr']
      ,'sfr.fr' : ['http://www.fdn.fr']
      // Vente en ligne
      ,'amazon.' : ['https://openlibrary.org']
      ,'leboncoin.fr' : ['videgrenierdunet.fr', 'http://www.wannonce.com']
      ,'ebay.' : ['videgrenierdunet.fr', 'http://www.wannonce.com']
      // Portail généraliste
      ,'linternaute.' : ['http://www.lepiredunet.fr', 'http://www.lepiredunet.fr']
      // Portail pornographique
      ,'youporn.' : ['http://fuckforforestmovie.com']
      ,'pornhub.' : ['http://fuckforforestmovie.com']
      ,'xnxx.' : ['http://fuckforforestmovie.com']
      ,'xhamster.' : ['http://fuckforforestmovie.com']
      // informations
      ,'lemonde.' : ['http://www.acrimed.org']
      // Portail d'écoute musicale
      ,'deezer.' : ['http://www.musiqueapproximative.net']
    ...
    function go(url, fake) {
      ...
      if (/^\/?(c\/|d\/|z\/)/.test(url)) {
        iframe.src = url;
        favicon.src = '/c/sys/ico32/file.png';
        return;
      }
      ...
      /*$ajax.post('/c/app/catex.php?v=3', {url: trydest}).done(function(data) {
        ...
        //$error('ajax error: Cat Explorer can\'t check the url');
      });*/
    ...
    var errorUrl = ["facebook", "twitter", "yahoo", "baidu", "live.", "amazon.", "linkedin", "blogspot", "vk.com", "wordpress.com", "bing.com", "ebay", "pinterest", "instagram", "msn", "tumblr", "paypal.", "apple.", "imdb", "adcash", "neobux", "reddit", "4chan", "stackoverflow", "blogger", "adobe.", "vube.com", "thepiratebay", "netflix", "soundcloud"];
    function filter(url) {
    ...
    var data = {
      icon: '/c/sys/ico32/nyanexplorer.png',
      title: 'cat explorer',
      width: 800,
      height: 600,
      url: theurl,
      onopen: function(el) {
        ...
        scope._states.opened['catex'].loaded = true;
        scope._states.opened['catex'].nav = navInput;
        scope._states.opened['catex'].go = go;
      ...
      onclose: function(el) {
        scope._states.opened['catex'] = null;
      ...
      beforeMenu: nav,
      menu: [
         {name: '', icon: '/c/sys/ico32/home.png', action: function(){go(HOMEPAGE)}}
        //,{name: '', icon: '/c/sys/ico32/myspace.png', action: function(){go('/c/files/archiveorg/myspace/index.html', 'http://myspace.com')}}
        ,{name: 'Crew', icon: '/c/sys/ico32/folder.png', items: [
           {name: 'Jankenpopp', action: function(){go('http://jankenpopp.com')}}
          ,{name: 'Zombectro', action: function(){go('http://zombect.ro')}}
          ,{name: 'databit.me', icon: '/c/sys/ico32/databit.gif', action: function(){go('http://databit.me/')}}
          ,{name: 'FESTILAB', icon: '/c/sys/ico32/love.png', action: function(){go('http://freesson.com/festilab/index.php')}}
        ]}
        ,{name: 'Media', icon: '/c/sys/ico32/folder.png', items: [
          ...
          ,{name: 'Comics', icon: '/c/sys/ico32/folder.png', items: [
        ...
        ,{name: 'Net-Art', icon: '/c/sys/ico32/folder.png', items: [
        ...
        ,{name: 'Games', icon: '/c/sys/ico32/folder.png', items: [
        ...
        ,{name: 'P()rn', icon: '/c/sys/ico32/folder.png', items: [
        ...
        ,{name: 'Gif', icon: '/c/sys/ico32/folder.png', items: [
        ...
        ,{name: 'Tools', icon: '/c/sys/ico32/folder.png', items: [
        ...
        ,{name: 'INTERWEBZ', icon: '/c/sys/ico32/folder.png', items: [
        ...
        ,{name: 'VM', icon: '/c/sys/ico32/folder.png', items: [
    ...
    scope._states.opened['catex'].instance = $window(data);
  }
});